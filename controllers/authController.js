...
const createSendToken = (user, statusCode, res) => {
  ...
  // The following stops the pasword from being shown on new user signups.
...
exports.protect = catchAsync(async (req, res, next) => {
  let token;
  // 1) Getting token and chick if it ixists
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // 2)  Generate rtandom reset tokens
  const resetToken = user.createPasswordResetToken();
  // { validateBeforeSave: false }   makes sure teh passwordConfirm
  // does not run. It would fail because there paswordCOnfirm is not
  // passed here.
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Update changedPasswordAt property for teh user
